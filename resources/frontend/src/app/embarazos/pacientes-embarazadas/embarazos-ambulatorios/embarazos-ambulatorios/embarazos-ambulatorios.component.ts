...
import { EmbarazosAmbulatoriosService } from '../pacientes-embarazos-ambulatorios.service';
...
@Component({
  selector: 'embarazos-ambulatorios',
  templateUrl: './embarazos-ambulatorios.component.html',
  styleUrls: ['./embarazos-ambulatorios.component.css'],
  animations: [
    trigger('buttonInOut', [
      transition('void => *', [
        style({ opacity: '1' }),
...
export class EmbarazosAmbulatoriosComponent implements OnInit {
  ...
  constructor(
    ...
    private embarazosAmbulatoriosService: EmbarazosAmbulatoriosService,
  ...
  public loadFilterCatalogs(){
    ...
    this.embarazosAmbulatoriosService.obtenerCatalogos(carga_catalogos).subscribe(
      response => {
    ...
    for(const i in data){
      ...
      if(data[i]){
        ...
        if(i == 'numero_expediente'){
          if(this.filterForm.value == 'numero_expediente'){
            item.tooltip =  'Número de Expediente',
            item.tag = 'Número de Expediente: '+this.filterForm.value.numero_expediente;
          }else{
          ...
          if(this.filterForm.value == 'numero_cama'){
            item.tooltip =  'Número/Descripción de Cama',
            item.tag = 'Número/Descripción de la Cama: '+this.filterForm.value.numero_cama;
          }else{
            item.tooltip =  'Número/Descripción de Cama',
            item.tag = 'Número/Descripción de la Cama: '+this.filterForm.value.numero_cama;
          }
          ...
          item.tooltip = "Estado de Salud de Egreso: "+data[i].nombre.toUpperCase();
        }
          ...
          item.tag = data[i].nombre;
          item.tooltip = "Motivo del Egreso: "+data[i].nombre.toUpperCase();
        }
        else if(i == 'condicion_egreso_id'){
          item.tag = data[i].nombre;
          item.tooltip = "Condición del Egreso: "+data[i].nombre.toUpperCase();
        }
        else if(i == 'metodo_anticonceptivo_id'){
          item.tag = data[i].nombre;
          item.tooltip = "Método Anticonceptivo: "+data[i].nombre.toUpperCase();
        }
  ...
  public loadPacientesData(event?:PageEvent){
    ...
    if(this.servicios.length > 0){
      ...
      //   var element = this.servicios[index];
    ...
    this.embarazosAmbulatoriosService.getEmbarazosAmbulatorios(params).subscribe(
      response =>{
        if(response.error) {
  ...
  reporteEmbarazadas(){
    ...
    params.reporte = 'Pacientes Embarazadas';
          ...
          //this.sharedService.showSnackBar(errorMessage, null, 3000);
            ...
            reportWorker.onmessage().subscribe(
              data => {
                ...
                FileSaver.saveAs(data.data,'Embarazos Ambulatorios'+'('+moment(this.fechaActual).format('DD-MM-YYYY')+')' );
            ...
            reportWorker.postMessage({data:{items: response.data, fecha_actual: response.fecha_actual, config:config},reporte:'/reporte-embarazos-ambulatorios'});
        }
        this.isLoading = false;
      },
      errorResponse =>{
