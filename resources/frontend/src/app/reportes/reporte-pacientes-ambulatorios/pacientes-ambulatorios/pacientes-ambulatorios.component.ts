...
@Component({
  selector: 'reporte-pacientes-ambulatorios',
  templateUrl: './pacientes-ambulatorios.component.html',
  styleUrls: ['./pacientes-ambulatorios.component.css'],
  animations: [
    trigger('buttonInOut', [
      transition('void => *', [
        style({ opacity: '1' }),
...
export class PacientesAmbulatoriosComponent implements OnInit {
  ...
  constructor(
    ...
    private pacientesAmbulatoriosService: PacientesAmbulatoriosService,
  ...
  public loadFilterCatalogs(){
    ...
    this.pacientesAmbulatoriosService.obtenerCatalogos(carga_catalogos).subscribe(
      response => {
    ...
    for(let i in data){
      ...
      if(data[i]){
        ...
        if(i === 'numero_expediente'){
          if(this.filterForm.get('numero_expediente').value === 'numero_expediente'){
          ...
          if(this.filterForm.get('numero_expediente').value === 'numero_cama'){
          ...
          var desde = moment(this.filterForm.get('fecha_inicio').value).format('DD/MM/YYYY'); 
          item.tag = desde;
          item.tooltip = "Fecha de Ingreso (Desde): " + desde;
        }
        else if (i === 'fecha_fin') {
          var hasta = moment(this.filterForm.get('fecha_fin').value).format('DD/MM/YYYY')
          item.tag = hasta;
  ...
  public loadPacientesData(event?:PageEvent){
    ...
    for(const i in filterFormValues){
      ...
      if(filterFormValues[i]){
        ...
        // }
          ...
          const estado_actual = this.filterForm.get('estado_actual_id').value;
          params[i] = estado_actual.id;
        }
        else if (i == 'fecha_inicio') {
    ...
    this.pacientesAmbulatoriosService.getPacientesAmbulatorios(params).subscribe({
      ...
      complete:() =>{
        this.sharedService.showSnackBar("Pacientes Ambulatorios en Línea...", 'Cerrar', 4000);
      }
  ...
  toggleReportPanel(){
    ...
    this.reportTitle = 'Pacientes Ambulatorios.';
            ...
            //this.sharedService.showSnackBar(errorMessage, null, 3000);
              ...
              reportWorker.onmessage().subscribe(
                data => {
                  ...
                  FileSaver.saveAs(data.data,'Pacientes-Ambulatorios'+'('+moment(this.fechaActual).format('DD-MM-YYYY')+')' );
              ...
              reportWorker.postMessage({data:{items: response.data, fecha_actual: response.fecha_actual, config:config},reporte:'/reporte-pacientes-ambulatorios'});
          }
          this.isLoading = false;
        },
        error:(errorResponse: HttpErrorResponse) => {
