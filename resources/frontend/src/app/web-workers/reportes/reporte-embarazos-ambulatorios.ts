...
export class ReporteEmbarazosAmbulatorios {
      ...
      //console.log(LOGOS);
        ...
        const tabla_vacia = {
          table: {
            ...
            widths: [ 15, 50, 40, 40, 40, 40, 40, 40, 45, 45, 40, 50, 70, 40, 40, 40, 45, 60],
            margin: [0,0,0,0],
            body: [
              [
                ...
                {text: "APEO (Método Anticonceptivo)", style: 'cabecera'},
        ...
        let fecha_alta = "";
        let motivo_egreso = "";
        let condiciones_egreso = "";
        let metodo_anticonceptivo = "";
        let observaciones_egreso = "";
        //EDADES
        ...
        for(let i = 0; i < reportData.items.length; i++){
              ...
              if(paciente.ultima_atencion?.alta){
                

                fecha_alta            = new Intl.DateTimeFormat('es-ES', {year: 'numeric', month: '2-digit', day: '2-digit'}).format(new Date(paciente.ultima_atencion?.alta.fecha_alta));
                motivo_egreso         = (paciente.ultima_atencion?.alta.motivo_egreso != null) ? paciente.ultima_atencion?.alta.motivo_egreso.nombre : 'No Asignado';
                condiciones_egreso    = (paciente.ultima_atencion?.alta.condiciones_egreso != null) ? paciente.ultima_atencion?.alta.condiciones_egreso.nombre : 'No Asignado';
                metodo_anticonceptivo = (paciente.ultima_atencion?.alta.metodo_anticonceptivo != null) ? paciente.ultima_atencion?.alta.metodo_anticonceptivo.nombre : 'No Asignado';
                observaciones_egreso  = paciente.ultima_atencion?.alta.observaciones;
                ...
                datos.content[indice_actual].table.body.push([
                  ...
                  { text: condiciones_egreso, style: 'tabla_datos'},
                  { text: metodo_anticonceptivo, style: 'tabla_datos'},
                  { text: observaciones_egreso, style: 'tabla_datos'},
                ]);
                diagnosticos_ingreso = "";
                diagnosticos_actual = "";
                diagnosticos_alta = "";
