...
@Component({
  selector: 'list-camas',
  templateUrl: './list-camas.component.html',
  styleUrls: ['./list-camas.component.css'],
  animations: [
    trigger('buttonInOut', [
        transition('void => *', [
            style({opacity: '1'}),
...
export class ListComponentCamas implements OnInit {
  ...
  displayedColumns: string[] = ['no_cama','servicio', 'estatus_cama', 'tipo_cama', 'opciones'];
  dataSource: any = [];
  ...
  filterForm =  new FormGroup({
    ...
    tipo_cama             : new FormControl(undefined),
    servicio_id           : new FormControl(undefined),
    estatus_cama_id       : new FormControl(undefined)
  ...
  ngOnInit() {
    ...
    //this.filterForm.get('clues_id').patchValue(this.authClues.id);
  ...
  public loadFilterCatalogs(){
    ...
    this.CamasService.obtenerCatalogos(carga_catalogos).subscribe(
      response => {
        ...
        this.filteredCatalogs['estatus_cama']        = this.filterForm.controls['estatus_cama_id'].valueChanges.pipe(startWith(''),map(value => this._filter(value,'estatus_cama','nombre')));
  ...
  loadFilterChips(data){
    ...
    for(let i in data){
      if(data[i]){
        ...
        if(i == 'numero'){
          if(this.filterForm.get('numero').value == 'numero'){
            item.tooltip =  'Descripción/N° de la Cama:',
            item.tag = this.filterForm.value.numero;
          }else{
            item.tooltip =  'Descripción/N° de la Cama:',
            item.tag = this.filterForm.value.numero;
          }
  ...
  public loadCamasData(event?:PageEvent){
    ...
    for(const i in filterFormValues){
      ...
      if(filterFormValues[i]){
        ...
        if(i == 'numero'){
          params[i] = this.filterForm.value.numero;
        }else if(i == 'tipo_cama'){
          params[i] = this.filterForm.value.tipo_cama;
        }else if(i == 'servicio_id'){
    ...
    this.CamasService.getCamasList(params).subscribe(
      response =>{
        if(response.error) {
          const errorMessage = response.error.message;
          this.sharedService.showSnackBar(errorMessage, null, 3000);
        } else {
          if(response.catalogo_camas.total > 0){
            this.dataSource = response.catalogo_camas.data;
            this.resultsLength = response.catalogo_camas.total;
          }else{
            this.dataSource = [];
  ...
  openDialogForm(id = 0){
    ...
    const dialogRef = this.dialog.open(FormComponentCamas,dialogConfig);
  ...
  //   console.log(datos);
  //   const dialogRef = this.dialog.open(DetailsComponentPaciente, {
  //     width:  '1200px',
  //     height: '650px',
  //     data: {datos: datos}
  //   });
  ...
  confirmDeleteCama(id = ''){
    const dialogRef = this.dialog.open(ConfirmActionDialogComponent, {
      width: '500px',
      data: {dialogTitle:'Eliminar Cama',dialogMessage:'¿Esta seguro de Eliminar la cama?',btnColor:'warn',btnText:'Eliminar'}
    });

    dialogRef.afterClosed().subscribe(reponse => {
      if(reponse){
        this.CamasService.deleteCama(id).subscribe(
          response => {
  ...
  toggleReportPanel(){
    ...
    this.reportTitle = 'Censo de Camas Hospitalario';
  ...
  reporteCamas(){
    ...
    params = { reporte: 'camas', clues: this.authClues ? this.authClues.id : '' };
          ...
          //this.sharedService.showSnackBar(errorMessage, null, 3000);
            ...
            reportWorker.onmessage().subscribe(
              data => {
                ...
                FileSaver.saveAs(data.data,'Reporte de Camas'+' '+'('+this.fechaActual+')' );
            ...
            reportWorker.postMessage({data:{items: response.catalogo_camas, config:config},reporte:'/reporte-camas'});
        }
        this.isLoading = false;
      },
      errorResponse =>{
  ...
  tarjetaInformativa (obj){
      ...
      reportWorker.postMessage({data:{items: obj, config:config},reporte:'/tarjeta-informativa'});
      this.isLoading = false;
  }
