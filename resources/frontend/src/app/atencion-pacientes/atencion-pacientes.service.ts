...
export class AtencionPacientesService {

  url                                     = `${environment.base_url}/atencion-pacientes-web`;
  url_lista_atencion_pacientes            = `${environment.base_url}/lista-atencion-pacientes-web`;                       
  ...
  getPacientesList(payload):Observable<any> {
    return this.http.post<any>(this.url_lista_atencion_pacientes, payload).pipe(
      map( response => {
        return response;
      })
    );
